...
btn.forEach(function(e){
	...
	console.log("선택한 맴버십 가격은 : "+ev.target.parentElement.querySelector('.mbsprice').innerText.substring(1));
	// 들고가야할 맴버쉽 코드번호를 서치
	const mbscode = ev.target.parentElement.querySelector('input[name="mbscode"]').value;
	console.log("선택하신 맴버십 코드는 : "+mbscode);
	...
	let popup = window.open("payment.do?value="+msPrice+"&orderName="+msName+"&customerName="+memberId, "Join Membership", "width=640, height=400");
	function closePop(){popup.close();if(popup.close){console.log('popup close!'); location.reload();}} // 팝업창 닫히면 바로 페이지 리로드
